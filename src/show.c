...
int action_show(int argc, char **argv)
{
    ...
    printf("\nHeader hash:  %s\n", hash_str);
    bpak_bin2hex(pkg.header.payload_hash,
                 hash_size,
                 hash_str,
                 sizeof(hash_str));
    printf("Payload hash: %s\n", hash_str);

    if (memcmp(pkg.header.payload_hash,
               payload_hash_copy,
               BPAK_HASH_MAX_LENGTH) != 0) {
        printf("Warning: Computed payload hash does not match payload hash in "
               "header\n");
    }
