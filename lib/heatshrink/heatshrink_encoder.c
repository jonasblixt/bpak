                                     ...
                                     size_t size, size_t *input_size)
{
    ...
    *input_size = cp_sz;
    ...
    LOG("-- sunk %u bytes (of %zu) into encoder at %d, input buffer now has "
        "%u\n",
        cp_sz,
        size,
        write_offset,
...
static void do_indexing(heatshrink_encoder *hse) { (void)hse; }
...
static uint16_t find_longest_match(heatshrink_encoder *hse, uint16_t start,
                                   ...
                                   uint16_t *match_length)
{
    LOG("-- scanning for match of buf[%u:%u] between buf[%u:%u] (max %u "
        "bytes)\n",
        end,
        end + maxlen,
        start,
