...
static void fill_with_pseudorandom_letters(uint8_t *buf, uint16_t size,
                                           uint32_t seed)
{
                                         ...
                                         int log_lvl)
{
    ...
    while (sunk < input_size) {
        ASSERT(heatshrink_encoder_sink(&hse,
                                       &input[sunk],
                                       input_size - sunk,
                                       &count) >= 0);
        sunk += count;
    ...
    while (sunk < compressed_size) {
        ASSERT(heatshrink_decoder_sink(&hsd,
                                       &comp[sunk],
        ...
        if (input[i] != decomp[i]) {
            ...
            if (0) {
                for (size_t j = 0; j <= /*i*/ input_size; j++) {
                    printf("in[%zd] == 0x%02x ('%c') => out[%zd] == 0x%02x "
                           "('%c')\n",
                           j,
                           input[j],
